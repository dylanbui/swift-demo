//
//  DbSearchTextField.swift
//  SwiftApp
//
//  Created by Dylan Bui on 7/30/19.
//  Copyright © 2019 Propzy Viet Nam. All rights reserved.
//  Base on (v1.2.2) : https://github.com/apasccon/SearchTextField

import UIKit

open class DbSearchTextField: UITextField
{
    ////////////////////////////////////////////////////////////////////////
    // Public interface
    ...
    /// Maximum width of the results list
    open var maxResultsListWidth = 0
    ...
    /// Set your custom visual theme, or just choose between pre-defined DbSearchTextFieldTheme.lightTheme() and DbSearchTextFieldTheme.darkTheme() themes
    open var theme = DbSearchTextFieldTheme.lightTheme() {
        didSet {
    ...
    open var startVisibleWithoutInteraction = false {
        didSet {
            if startVisibleWithoutInteraction {
                // -- DucBui: Bo hien thi khi vua moi load --
                // -- Neu muon hien TableView khi load --
                // ta chi can set startVisibleWithoutInteraction = true
                // truoc khi set gia tri self.filterStrings || self.filterItems
                // textFieldDidChange()
            }
        }
    }
    
    /// Set an array of DbSearchTextFieldItem's to be used for suggestions
    open func filterItems(_ items: [DbSearchTextFieldItem]) {
        filterDataSource = items
    }
    
    /// Set an array of strings to be used for suggestions
    open func filterStrings(_ strings: [String]) {
        var items = [DbSearchTextFieldItem]()
        
        for value in strings {
            items.append(DbSearchTextFieldItem(title: value))
        }
    ...
    open var itemSelectionHandler: DbSearchTextFieldItemHandler?
    ...
    /// Set the results list's footer
    open var resultsListFooter: UIView?
    
    /// Set the results list's section header
    open var resultsListSectionHeader: UIView?
    
    /// Set the results list's section footer
    open var resultsListSectionFooter: UIView?
    ...
    fileprivate var direction: DbSearchTextFieldDirection = .down
    
    fileprivate var fontConversionRate: CGFloat = 0.75
    fileprivate var keyboardFrame: CGRect?
    fileprivate var timer: Timer? = nil
    fileprivate var placeholderLabel: UILabel?
    fileprivate static let cellIdentifier = "APDbSearchTextFieldCell"
    fileprivate let indicator = UIActivityIndicatorView(activityIndicatorStyle: .gray)
    fileprivate var maxTableViewSize: CGFloat = 0
    
    fileprivate var filteredResults = [DbSearchTextFieldItem]()
    fileprivate var filterDataSource = [DbSearchTextFieldItem]() {
        didSet {
    ...
    override open func willMove(toSuperview newSuperview: UIView?) {
        ...
        self.addTarget(self, action: #selector(DbSearchTextField.textFieldDidChange), for: .editingChanged)
        self.addTarget(self, action: #selector(DbSearchTextField.textFieldDidBeginEditing), for: .editingDidBegin)
        self.addTarget(self, action: #selector(DbSearchTextField.textFieldDidEndEditing), for: .editingDidEnd)
        self.addTarget(self, action: #selector(DbSearchTextField.textFieldDidEndEditingOnExit), for: .editingDidEndOnExit)
        
        NotificationCenter.default.addObserver(self, selector: #selector(DbSearchTextField.keyboardWillShow(_:)), name: NSNotification.Name.UIKeyboardWillShow, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(DbSearchTextField.keyboardWillHide(_:)), name: NSNotification.Name.UIKeyboardWillHide, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(DbSearchTextField.keyboardDidChangeFrame(_:)), name: NSNotification.Name.UIKeyboardDidChangeFrame, object: nil)
    }
    ...
    fileprivate func buildSearchTableView() {
        if let tableView = tableView, let shadowView = shadowView {
            ...
            tableView.tableFooterView = resultsListFooter
            if forceRightToLeft {
    ...
    // MARK: - Re-set frames and theme colors
    // Chinh chieu cao, frame cua uitableview
    fileprivate func redrawSearchTableView() {
        ...
        if let tableView = tableView {
            ...
            guard var frame = self.superview?.convert(self.frame, to: nil) else { return }
            if let parentView = self.containerView {
            ...
            if self.direction == .down {
                ...
                // -- DucBui: Da chinh nhieu so voi phien ban goc, dung self.containerView de xac dinh vi tri --
                // -- Chua xu ly tinh toan offsetY khi set maxResultsListWidth --
                var tableViewFrame = CGRect(
                    x: 0, y: 0,
                    width: self.maxResultsListWidth != 0 ? CGFloat(self.maxResultsListWidth) : frame.size.width,
                    height: tableHeight)
                
                tableViewFrame.origin = self.convert(tableViewFrame.origin, to: nil)
                tableViewFrame.origin = frame.origin
    ...
    @objc open func textFieldDidChange() {
        ...
        timer?.invalidate()
        timer = Timer.scheduledTimer(timeInterval: typingStoppedDelay, target: self, selector: #selector(DbSearchTextField.typingDidStop), userInfo: self, repeats: false)
    ...
    @objc open func textFieldDidEndEditingOnExit() {
        if let firstElement = filteredResults.first {
            // -- When touch return buton, dont return first item --
...
extension DbSearchTextField: UITableViewDelegate, UITableViewDataSource {
    
    public func tableView(_ tableView: UITableView, heightForHeaderInSection section: Int) -> CGFloat {
        return self.resultsListSectionHeader != nil ? self.resultsListSectionHeader!.frame.size.height : 0.0
    }
    
    public func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        return self.resultsListSectionHeader
    }
    
    public func tableView(_ tableView: UITableView, heightForFooterInSection section: Int) -> CGFloat {
        return self.resultsListSectionFooter != nil ? self.resultsListSectionFooter!.frame.size.height : 0.0
    }
    
    public func tableView(_ tableView: UITableView, viewForFooterInSection section: Int) -> UIView? {
        return self.resultsListSectionFooter
    }
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        var cell = tableView.dequeueReusableCell(withIdentifier: DbSearchTextField.cellIdentifier)
        
        if cell == nil {
            cell = UITableViewCell(style: .subtitle, reuseIdentifier: DbSearchTextField.cellIdentifier)
        }
...
public struct DbSearchTextFieldTheme {
    ...
    public static func lightTheme() -> DbSearchTextFieldTheme {
        return DbSearchTextFieldTheme(cellHeight: 30,
    ...
    public static func darkTheme() -> DbSearchTextFieldTheme {
        return DbSearchTextFieldTheme(cellHeight: 30,
...
open class DbSearchTextFieldItem {
    // Private vars
...
public typealias DbSearchTextFieldItemHandler = (_ filteredResults: [DbSearchTextFieldItem], _ index: Int) -> Void

////////////////////////////////////////////////////////////////////////
// Suggestions List Direction

enum DbSearchTextFieldDirection {
    case down
    case up
}
