...
/// declared with `let`
public final class DbHTTPResult : NSObject {
    ...
    public var reason: String {
        if let code = self.statusCode, let text = dbStatusCodeDescriptions[code] {
            return text
        }
...
typealias DbTaskID = Int
public typealias DbCredentials = (username: String, password: String)
public typealias DbTaskProgressHandler = (DbHTTPProgress) -> Void
typealias DbTaskCompletionHandler = (DbHTTPResult) -> Void

struct DbTaskConfiguration {
    let credential: DbCredentials?
    let redirects: Bool
    let originalRequest: URLRequest?
    var data: Data
    let progressHandler: DbTaskProgressHandler?
    let completionHandler: DbTaskCompletionHandler?
}

public struct DbHTTPSessionDefaults {
...
let httpErrorDomain = "net.justhttp.Just"

public protocol RequestAdaptor {
    func request(
        ...
        urlQuery: String?,
        requestBody: Data?,
        asyncProgressHandler: DbTaskProgressHandler?,
        asyncCompletionHandler: ((DbHTTPResult) -> Void)?
        ) -> DbHTTPResult
    
    init(session: URLSession?, defaults: DbHTTPSessionDefaults?)
}

public struct ConnectionOf<Adaptor: RequestAdaptor> {
    let adaptor: Adaptor
    public init(session: URLSession? = nil,
                defaults: DbHTTPSessionDefaults? = nil)
    {
...
extension ConnectionOf {
    ...
    public func request(
        ...
        asyncCompletionHandler: ((DbHTTPResult) -> Void)? = nil
        ) -> DbHTTPResult {
...
public final class HTTP: NSObject, URLSessionDelegate, RequestAdaptor {
                ...
                defaults: DbHTTPSessionDefaults? = nil)
    {
        ...
        if let initialDefaults = defaults {
            ...
            self.defaults = DbHTTPSessionDefaults()
        }
    }
    
    var taskConfigs: [DbTaskID: DbTaskConfiguration]=[:]
    var defaults: DbHTTPSessionDefaults!
    ...
    func makeTask(_ request: URLRequest, configuration: DbTaskConfiguration)
        -> URLSessionDataTask?
    {
    ...
    func synthesizeMultipartBody(_ data: [String: Any], files: [String: DbHTTPFile])
        -> Data?
    {
        var body = Data()
    ...
    public func request(
        ...
        requestBody: Data?,
        asyncProgressHandler: DbTaskProgressHandler?,
        asyncCompletionHandler: ((DbHTTPResult) -> Void)?) -> DbHTTPResult {
        
        let isSynchronous = asyncCompletionHandler == nil
        let semaphore = DispatchSemaphore(value: 0)
        var requestResult: DbHTTPResult = DbHTTPResult(data: nil, response: nil,
        ...
        {
            let erronousResult = DbHTTPResult(data: nil, response: nil,
        ...
        let config = DbTaskConfiguration(
            credential: auth,
            redirects: redirects,
...
public let DbHTTP = ConnectionOf<HTTP>()
